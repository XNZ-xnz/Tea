...
- Wine 来源（2026-07-23 实测定型）：DX11/通用 = Gcenx **macOS_Wine_builds**（WineHQ 官方构建；wine-crossover 仓库已消亡）+ DXMT 变体；DX12 = Gcenx **game-porting-toolkit**（gptk-wine 原装含 D3DMetal 3，实测 D3D12_OK）。GPTK 4 库需 CX24/25 代底座（实测 c0000142），只导入登记暂不启用。DXMT v0.80 vanilla wine 可带动（不再需要 CX 私有符号）。
