...
- **阶段**：✅ P0-P3 完成；**降级完成 + 环境重建完成（macOS 26.5.2 / 25F84，Rosetta/Xcode26/gh 全就位）**；🏆 **CEF 黑屏彻底根治，Steam 完整 UI 正常渲染（27beta 噩梦终结）**；🏆 **Against the Storm 进入可交互主菜单、并已进入局内世界地图（2026-07-24 下午，自建 DXVK，首款「能点自己界面」的游戏）**；⚠️ 帧率数字待重测（测量期间机器被 accountsd/contactsd/distnoted 拖垮，见「帧率结论修订」段）
...
**下一步**：重启机器 → 重跑对照组拿可信基线 → 若局内仍受限而 GPU 闲置，方向转向
「游戏自身 CPU 逻辑在 Rosetta 下的开销」，而非图形转译层。
