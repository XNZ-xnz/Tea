...
### 帧率攻坚：4.8 → 22.6 FPS（同日续，4.7 倍）

**根因：游戏画质档是从产品负责人 RTX 4070 主机经 Steam Cloud 同步过来的 `High`**。
（wine 注册表里的 `UnityGraphicsQuality=0` 是幌子——AoTS 不用 Unity 质量等级，用自己的设置系统，
在游戏内 OPTIONS→VIDEO 里。这条对所有「读云端设置的游戏」通用：**Mac 用户第一次跑必须先降画质**。）

**实测矩阵**（DXVK HUD `fps,gpuload,drawcalls,submissions` 屏显读数）：

| 配置 | FPS | Draw calls | Render passes | GPU |
|---|---|---|---|---|
| High + 2560×1664（云端继承态） | 4.8-6.5 | 11360 | 32 | **100%** |
| High + **1710×1068**（像素量降到 43%） | — | — | — | — |
| **Low** + 1710×1068 | 19.2-19.6 | 5659 | 24 | 46-53% |
| **Low + 2560×1664（最终态）** | **22.6** | 5633 | 24 | **44%** |

**两条定性结论**：
1. **分辨率不是杠杆**：像素量砍到 43%，FPS 19.6→19.2 纹丝不动，GPU 反而 46%→53%。
   **不是填充率瓶颈**。最终态干脆用回原生 2560×1664，反而是全场最高的 22.6 FPS。
2. **draw call 数是唯一有效杠杆**：High→Low 把 draw call 从 11360 砍到 5633（菜单场景 1090 棵树无合批），
   FPS 直接 4.7 倍，GPU 从打满降到 44%。**瓶颈是 DXVK→MoltenVK→Metal 的每次绘制提交开销**。

**弹药：`MVK_CONFIG_USE_METAL_ARGUMENT_BUFFERS=1` — 判定有害，勿用**。
本意是降低描述符/提交开销，实测把游戏钉死在加载画面 0.5 FPS（43 个 draw call 都跑不动，画面 6 秒零变化）。
**更坑的是它会污染 DXVK 着色器缓存**：去掉环境变量重启仍然 0.5 FPS，必须删掉
`prefix/drive_c/users/xnz/AppData/Local/dxvk/<hash>.dxvk.{bin,lut}` 才恢复。

**低帧率下的自动化纪律（新增）**：5 FPS 时游戏每 200ms 才轮询一次输入，CGEvent 点击必须
**先移动停留 600ms + 按住 500-700ms** 才能被采样到；按下-抬起间隔 80ms 的常规点击会被整个漏掉
（踩坑两次，误以为坐标算错）。工具在 scratchpad/aots/click.swift。

### ⚠️ 帧率结论修订（同日续，第二轮实验后）

**上面「22.6 FPS」与「draw call 是唯一杠杆」两条要打折，原因是测量环境被污染 + 方法有缺陷。**
按可信度重新分档：

**站得住（都是 HUD 计数器，与系统负载无关）**：
- 画质 High→Low：draw calls **11360→5633**、render passes 32→24、GPU **100%→44%**。方向确定。
- **局内世界地图：draw calls 仅 255、GPU 0%，帧率仍在 20 上下**——draw call 掉 22 倍、GPU 全闲，
  帧率不动。**证明图形栈不是局内瓶颈**，「draw call 是唯一杠杆」只在 High 档（GPU 真打满时）成立。
- `MVK_CONFIG_USE_METAL_ARGUMENT_BUFFERS=1` 有害且污染 DXVK 着色器缓存（效应极大，不受噪声影响）。
- 关 vsync 无效：`dxgi.syncInterval=0` 让 present mode 变 `IMMEDIATE_KHR`，8 采样中位 17.5，
  对照 FIFO 中位 17.45。**排除 present/vsync 锁**（此前「21.5≈60/3 是 FIFO 锁」的推断已证伪）。
- `ROSETTA_ADVERTISE_AVX=1` 无收益：中位 14.4 < 对照 17.45，方差更大。

**不可信（全部绝对帧率数字，含此前标题里的 22.6）**：测量期间机器处于严重降级状态——
`accountsd` 持续 134-259% CPU、`contactsd` 88-100% + 767MB、`distnoted` 泄漏到 **4.6GB**、
交换分区 6144MB 用掉 5555MB（90%）、另有 Claude 桌面版占 5GB/55%。
最后游戏被挤到 **0.7% CPU** 直接卡死。**需重启机器后重测**。

**测量方法论（血泪，三个坑全踩过）**：
1. **Unity 窗口失焦即暂停整个循环**。任何非前台状态下测的帧率都是垃圾。判据：游戏进程 CPU
   掉到个位数 % 就是暂停了。
2. **`screencapture -l <windowID>` 对 wine 窗口返回陈旧缓存表面**（本文件前面已记过一次，
   第二轮又踩）。全屏 `screencapture -x -o` 才是活的。**判活性只能用全屏截图或看 CPU**。
3. **激活工具本身会抢焦点**：`activate-any.swift` 是 AppKit 进程，在采样循环里每次调用反而破坏焦点。
   正解是用**一次点击**拿焦点，且落点要避开 Steam 窗口（x120-1379/y58-869）和 Dock（底部约 90 点）。
4. `pgrep -f "<游戏exe名>"` 会同时匹配到 `UnityCrashHandler64.exe` 和自身 shell，测 CPU 前必须过滤。
5. 单次读数不可靠，帧率在同场景同 draw call 下浮动极大——**必须多采样取中位**。
   工具：`scratchpad/aots/hudstrip.swift`（把 N 张截图左上角 HUD 裁出来纵向拼成一张，一次读完）。

### 🏆 终局（重启后干净环境，2026-07-24 傍晚）：真实玩法贴满帧，可玩实锤

重启清掉系统故障进程后全链路重测（8 采样 + 游戏区裁剪 md5 双重活性验证，全部通过）：

| 场景 | FPS（8采样） | Draw calls | GPU |
|---|---|---|---|
| 主菜单（Low 画质） | 33-43 | 5633 | 76-83% |
| 世界地图 | **60.0 锁满** | 257 | 52-70% |
| **局内聚落（Blightwatch 真实玩法，产品负责人同时在操作）** | **55.6-60.0，中位 57.6**（一次 43.1） | 662-757 | 64-99% |

**产品负责人亲手试玩确认可玩。** 此前 17.5/22.6 全是被拖垮机器上的产物；「53.0 干净基线」也是
冻结帧假读数（碰巧接近真值但方法无效）。徽章建议：**Playable+（M4 base，60Hz 内屏贴满）**，
正式 compat 报告待写。

**新增两个工程事实**：
1. **失焦启动 → Unity 冻死在主菜单**（两次复现：经 Steam 启动时焦点在 Terminal/Finder 即触发；
   冻结不可恢复，杀进程重启并**在加载期间用 osascript 循环保持前台**即正常）。这要进 recipe 已知问题
   与 P4 启动器设计——Tea 启动游戏后应把游戏窗口带到前台。
2. **判活性唯一可靠方法：全屏截图后裁游戏区比 md5**。全屏字节数会被菜单栏时钟污染（假活），
   窗口级 `-l` 是陈旧表面（假死），冻结帧上的 HUD 数字会撒谎（53.0 事件）。
   fps-bench.sh 已内置活性校验。
